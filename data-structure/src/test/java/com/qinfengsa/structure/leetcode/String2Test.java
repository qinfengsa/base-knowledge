...
public class String2Test {
     ...
     *
     * <p>当一个字符串满足如下条件时，我们称它是 美丽的 ：
     *
     * <p>所有 5 个英文元音字母（'a' ，'e' ，'i' ，'o' ，'u'）都必须 至少 出现一次。 这些元音字母的顺序都必须按照 字典序 升序排布（也就是说所有的 'a' 都在
     * 'e' 前面，所有的 'e' 都在 'i' 前面，以此类推） 比方说，字符串 "aeiou" 和 "aaaaaaeiiiioou" 都是 美丽的 ，但是 "uaeio" ，"aeoiu"
     * 和 "aaaeeeooo" 不是美丽的 。
     *
     * <p>给你一个只包含英文元音字母的字符串 word ，请你返回 word 中 最长美丽子字符串的长度 。如果不存在这样的子字符串，请返回 0 。
     *
     * <p>子字符串 是字符串中一个连续的字符序列。
     *
     * <p>示例 1：
     *
     * <p>输入：word = "aeiaaioaaaaeiiiiouuuooaauuaeiu" 输出：13 解释：最长子字符串是 "aaaaeiiiiouuu" ，长度为 13 。 示例
     * 2：
     *
     * <p>输入：word = "aeeeiiiioooauuuaeiou" 输出：5 解释：最长子字符串是 "aeiou" ，长度为 5 。 示例 3：
     *
     * <p>输入：word = "a" 输出：0 解释：没有美丽子字符串，所以返回 0 。
     *
     * <p>提示：
     *
     * <p>1 <= word.length <= 5 * 105 word 只包含字符 'a'，'e'，'i'，'o' 和 'u' 。
     *
     * @param word
     * @return
     */
    ...
    public void longestBeautifulSubstring() {
        ...
        logResult(longestBeautifulSubstring(word));
    }

    /**
     * 5730. 将所有数字用字符替换
     *
     * <p>给你一个下标从 0 开始的字符串 s ，它的 偶数 下标处为小写英文字母，奇数 下标处为数字。
     *
     * <p>定义一个函数 shift(c, x) ，其中 c 是一个字符且 x 是一个数字，函数返回字母表中 c 后面第 x 个字符。
     *
     * <p>比方说，shift('a', 5) = 'f' 和 shift('x', 0) = 'x' 。 对于每个 奇数 下标 i ，你需要将数字 s[i] 用 shift(s[i-1],
     * s[i]) 替换。
     *
     * <p>请你替换所有数字以后，将字符串 s 返回。题目 保证 shift(s[i-1], s[i]) 不会超过 'z' 。
     *
     * <p>示例 1：
     *
     * <p>输入：s = "a1c1e1" 输出："abcdef" 解释：数字被替换结果如下： - s[1] -> shift('a',1) = 'b' - s[3] ->
     * shift('c',1) = 'd' - s[5] -> shift('e',1) = 'f' 示例 2：
     *
     * <p>输入：s = "a1b2c3d4e" 输出："abbdcfdhe" 解释：数字被替换结果如下： - s[1] -> shift('a',1) = 'b' - s[3] ->
     * shift('b',2) = 'd' - s[5] -> shift('c',3) = 'f' - s[7] -> shift('d',4) = 'h'
     *
     * <p>提示：
     *
     * <p>1 <= s.length <= 100 s 只包含小写英文字母和数字。 对所有 奇数 下标处的 i ，满足 shift(s[i-1], s[i]) <= 'z' 。
     *
     * @param s
     * @return
     */
